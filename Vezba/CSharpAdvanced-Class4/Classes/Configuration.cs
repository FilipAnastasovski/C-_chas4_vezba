...
{     
    ...
    public class Configuration : Item, IPrice, IDiscont
    {
        ...
        //
        public void RemovePartFromConfiguration(Part part)
        {
            _parts.Remove(part);
        }

        public void RemovePartFromConfiguration(Module module)
        {
        ...
        public double GetPrice()
        {
            ...
            double priceParts = 0.0;
            double priceM = 0.0;

            foreach (var item in _parts)
            {
                priceParts += item.Price * item.Quantity;
            }

            foreach (var item in _modules)
            {
                priceM += item.GetPrice() ;
            }

            return priceParts + priceM;
